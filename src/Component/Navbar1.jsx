...
// //               <li className="nav-item me-2 search-container">
// //                 <input
// //                   type="text"
...
//   ? state.filter((el) => el && el.categoryname.toLowerCase().includes(search.toLowerCase()))
//   : [];
...
//                     {filterproducts.length > 0 ? (
//                       filterproducts.map((el) => (
//                         <Link to={`/product/${el._id}`} className="dropdown-item text-dark py-2 px-3">
//                           {el.categoryname}
//                         </Link>
//                       ))
//                     ) : (
...
export default function Navbar1() {
  ...
  async function fetchdata() {
    try {
      const res = await axios.get("https://titan-backend-zro7.onrender.com/category/get");
      setState(Array.isArray(res.data) ? res.data : []);
    } catch (error) {
      console.error("Error fetching data:", error);
      setState([]);
