...
export default function Cart() {
  ...
  const calculateTotalPrice = () => {
    return cart.reduce((total, item) => {
      // Ensure productId exists before accessing its price
    ...
    <div className="cart-container">
      ...
      {cart.length > 0 ? (
        <>
          <div className="cart-items">
            {cart.map((el) => {
              const product = el.productId || {}; // Ensure productId exists
              const image = product.image || ""; // Default to empty string if image doesn't exist
              const name = product.name || "Unnamed Product"; // Default name
              const price = product.price || "N/A"; // Default price

              return (
                <div className="cart-item" key={el._id}>
                  {image && <img src={image} alt={name} className="cart-item-image" />}
                  <div className="cart-item-details">
                    <h3>{name}</h3>
                    <p>Price: {price}</p>
                    <div className="quantity-controls">
